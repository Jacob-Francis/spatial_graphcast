...
smoothing_kernel_radius_in_metres = 1*1000
...
generate_smoothing_data_for_the_kdtree_based_approach_and_write_it_to_the_disk(lat, lon, smoothing_kernel_radius_in_metres, smoothing_data_folder)
...
smoothing_data_pointer = read_smoothing_data_for_the_kdtree_based_approach_from_binary_file_ctypes(smoothing_data_folder, smoothing_kernel_radius_in_metres)
#smoothing_data_pointer = read_smoothing_data_from_binary_file(smoothing_data_folder, 1*1000, len(lat))

# calculate the smoothed field
values1_smoothed = smooth_field_using_smoothing_data_for_the_kdtree_approach(area_size, values1, smoothing_data_pointer)
values2_smoothed = smooth_field_using_smoothing_data_for_the_kdtree_approach(area_size, values2, smoothing_data_pointer)

# calulate CSSS_p for p = 1.5
p = 2
[CSSS_p_value, CSSS_p_gradient] = calculate_CSSSp_value_with_gradient_using_kdtree_smoothing_data(area_size, values1, values2, smoothing_data_pointer, p)
...
matplotlib.pyplot.savefig('input_data.png')
...
gl = ax.gridlines()
gl.xlocator = mticker.FixedLocator(list(np.arange(-180,180,20)))
matplotlib.pyplot.savefig(f'smoothed_data_{smoothing_kernel_radius_in_metres}.png')
matplotlib.pyplot.show()
matplotlib.pyplot.close()
...
img = matplotlib.pyplot.imshow(np.reshape(CSSS_p_gradient,f1_netcdf.shape), transform=cartopy.crs.PlateCarree(), interpolation='nearest', origin='lower', extent=(0, 360, -90, 90),  vmax= np.max(np.abs(CSSS_p_gradient)), vmin=-np.max(np.abs(CSSS_p_gradient)), cmap ="bwr")
cb = fig.colorbar(img, extend='both', shrink=0.5)
ax.coastlines(resolution='110m', color='grey', linestyle='-', alpha=1)
gl = ax.gridlines()
gl.xlocator = mticker.FixedLocator(list(np.arange(-180,180,20)))
matplotlib.pyplot.savefig(f'csss_grad_{smoothing_kernel_radius_in_metres}.png')
matplotlib.pyplot.show()
matplotlib.pyplot.close()